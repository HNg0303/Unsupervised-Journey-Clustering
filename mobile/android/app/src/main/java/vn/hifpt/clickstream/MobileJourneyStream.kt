...
/** Thread-safe, multi-session incremental journey inference. */
class MobileJourneyStream(
    ...
    private data class State(val events: MutableList<ClickstreamEvent> = mutableListOf(), var journeyNumber: Int = 1)
    ...
    fun append(event: ClickstreamEvent, scoreProvisional: Boolean = true): StreamUpdate {
        if (event.sessionId.isBlank()) return StreamUpdate(emptyList(), null, listOf("missing_session_id"))
        if (MobileJourneyPreprocessor.eventTimeMillis(event) <= 0L) {
        ...
        state.events += event
        state.events.sortWith(compareBy<ClickstreamEvent> { MobileJourneyPreprocessor.eventTimeMillis(it) }.thenBy { it.sourceIndex })
    ...
    fun snapshotProvisional(sessionId: String? = null): MobilePrediction? {
        val entry = if (sessionId == null) states.entries.firstOrNull() else states[sessionId]?.let { java.util.AbstractMap.SimpleEntry(sessionId, it) }
        return entry?.let { model.scoreEvents(journeyId(it.key, it.value), it.value.events.toList(), provisional = true) }
    }
    ...
    /** Compatibility helper for the single-session demo. */
    @Synchronized
    fun flush(): MobilePrediction? = states.keys.firstOrNull()?.let { flush(it) }
    ...
    private fun finalize(sessionId: String, state: State, reason: String): MobilePrediction? {
        ...
        val result = model.scoreEvents(journeyId(sessionId, state), state.events.toList(), provisional = false, boundaryReason = reason)
        state.events.clear()
